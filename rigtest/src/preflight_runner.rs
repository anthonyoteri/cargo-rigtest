...
async fn run_probe(p: &Probe) -> ProbeOutcome {
    match &p.kind {
        ...
        ProbeKind::Dns { host } => run_dns(host, p.timeout).await,
        #[cfg(feature = "http-client")]
        ProbeKind::Http { url, expect } => run_http(url, expect, p.timeout).await,
        #[cfg(all(feature = "ssh-client", unix))]
        ProbeKind::Ssh { dest, command } => run_ssh(dest, command, p.timeout).await,
        ProbeKind::Custom { run } => run_custom(run, p.timeout).await,
    }
}
...
async fn run_dns(host: &str, deadline: Duration) -> ProbeOutcome {
    // `tokio::net::lookup_host` wants a `host:port` value; the port plays
    // no role here — we only consult whether at least one address came
    // back — so we tack on `:0`. The owned `String` keeps the value alive
    // across the await boundary that `timeout` introduces.
    let target = format!("{host}:0");
    match timeout(deadline, lookup_host(target)).await {
        Ok(Ok(mut iter)) => {
        ...
        Ok(Err(e)) => ProbeOutcome::Failed(format!("resolving {host} failed: {e}")),
        Err(_) => ProbeOutcome::Failed(format!(
            "resolving {host} timed out after {:.3}s",
            deadline.as_secs_f64()
        )),
    }
}
...
async fn run_http_with(
    ...
    let mut builder = reqwest::ClientBuilder::new().timeout(deadline);
    ...
    match timeout(deadline, client.get(url).send()).await {
        ...
        Ok(Err(e)) => ProbeOutcome::Failed(format!("GET {url} failed: {e}")),
        Err(_) => ProbeOutcome::Failed(format!(
            "GET {url} timed out after {:.3}s",
            deadline.as_secs_f64()
        )),
    }
}
...
#[cfg(all(feature = "ssh-client", unix))]
async fn run_ssh(dest: &str, command: &str, deadline: Duration) -> ProbeOutcome {
    let configurator: Option<SshConfigurator> = crate::registry::RIG_SSH_CLIENT_CONFIGURATOR
        .first()
        .map(|e| e.configure_fn);
    run_ssh_with(dest, command, deadline, configurator).await
}
...
async fn run_ssh_with(
    ...
    // The body is an async block returning `Result<(), String>` so we
    // can mix openssh errors and our own exit-status string into one
    // value that `timeout` wraps neatly.
    let fut = async move {
        let session = builder
            .connect(dest)
            .await
        ...
        if !status.success() {
            return Err(format!("remote command {command:?} exited with {status}"));
        }
    ...
    match timeout(deadline, fut).await {
        Ok(Ok(())) => ProbeOutcome::Passed,
        Ok(Err(msg)) => ProbeOutcome::Failed(format!("ssh {dest}: {msg}")),
        Err(_) => ProbeOutcome::Failed(format!(
            "ssh {dest} timed out after {:.3}s",
            deadline.as_secs_f64()
        )),
    }
}

async fn run_custom(run: &CustomProbeFn, deadline: Duration) -> ProbeOutcome {
    let fut = (run)();
    match timeout(deadline, fut).await {
        Ok(Ok(())) => ProbeOutcome::Passed,
        Ok(Err(e)) => ProbeOutcome::Failed(format!("{e}")),
        Err(_) => ProbeOutcome::Failed(format!(
            "custom probe timed out after {:.3}s",
            deadline.as_secs_f64()
        )),
    }
}
...
mod tests {
    ...
    async fn dns_probe_passes_for_localhost() {
        ...
        let outcome = run_dns("localhost", Duration::from_secs(1)).await;
        assert!(
            matches!(outcome, ProbeOutcome::Passed),
            "expected pass, got {outcome:?}"
        );
    ...
    async fn dns_probe_fails_for_invalid_tld() {
        ...
        let outcome = run_dns("nonexistent.rigtest.invalid", Duration::from_secs(2)).await;
        assert!(
    ...
    async fn http_probe_passes_against_local_server() {
        ...
        let outcome = run_http(url, &ExpectStatus::Range(200..=299), Duration::from_secs(2)).await;
        assert!(
            matches!(outcome, ProbeOutcome::Passed),
            "expected pass, got {outcome:?}"
        );
    ...
    async fn http_probe_exact_status_match() {
        ...
        let server = spawn_oneshot_http(204).await;
        let url: &'static str = Box::leak(format!("http://{}/", server.addr).into_boxed_str());
        let outcome = run_http(url, &ExpectStatus::Exact(204), Duration::from_secs(2)).await;
        assert!(matches!(outcome, ProbeOutcome::Passed));
    }
    ...
    async fn http_probe_configurator_error_does_not_short_circuit_other_probes() {
        ...
        let (http_outcome, tcp_outcome) = tokio::join!(
            ...
            run_tcp(&tcp_target, Duration::from_secs(1)),
        );
