...
//! (`String`, `format!(...)`, stored as `Cow::Owned`) work, so probe
//! targets can be constructed dynamically without manual lifetime
//! juggling.
//!
...
pub struct Probe {
    ...
    pub kind: ProbeKind,
    /// Per-probe deadline applied by the runner.
    ///
...
/// and owned strings (`String`, `format!(...)`, stored as `Cow::Owned`)
/// work without manual lifetime juggling.
///
/// Fields may be added in future releases. The `#[non_exhaustive]`
/// attribute prevents external code from constructing this struct via
...
impl Preflight {
    ...
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().tcp("api", "127.0.0.1:8080");
    /// ```
    pub fn tcp(
        mut self,
    ...
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().env("home_is_set", "HOME");
    /// ```
    pub fn env(
        mut self,
        name: impl Into<Cow<'static, str>>,
        var: impl Into<Cow<'static, str>>,
    ) -> Self {
        self.probes.push(Probe {
            name: name.into(),
            kind: ProbeKind::Env {
    ...
    /// timeout is [`DEFAULT_PROBE_TIMEOUT`] (30 seconds); override
    /// per-probe with [`timeout`][Self::timeout].
    ///
    /// # Example
    ///
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().dns("dns_api", "api.example.com");
    /// ```
    pub fn dns(
        mut self,
        name: impl Into<Cow<'static, str>>,
    ...
    /// timeout (default [`DEFAULT_PROBE_TIMEOUT`], 30 seconds) with
    ...
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().http("api", "https://api.example.com/health");
    /// ```
    #[cfg(feature = "http-client")]
    pub fn http(
        mut self,
    ...
    /// [`DEFAULT_PROBE_TIMEOUT`], 30 seconds) with
    /// [`timeout`][Self::timeout].
    ///
    ...
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().ssh("bastion", "deploy@bastion.example.com");
    /// ```
    #[cfg(all(feature = "ssh-client", unix))]
    pub fn ssh(
        mut self,
        name: impl Into<Cow<'static, str>>,
        dest: impl Into<Cow<'static, str>>,
    ) -> Self {
        self.probes.push(Probe {
            name: name.into(),
            kind: ProbeKind::Ssh {
    ...
    /// named primitives don't cover.
    ///
    /// `factory` is invoked once when the probe runs and must return a
    /// `Send` future that resolves to `Ok(())` on success or to an error
    /// on failure.
    ///
    /// Unlike every other primitive, `custom` carries **no
    /// framework-imposed timeout** by default — the framework has zero
    /// insight into what work the closure does, so any default would be
    /// wrong for some legitimate use case. The closure owns its own
    /// discipline. Set one with [`timeout`][Self::timeout] if you want
    /// the runner to enforce a deadline.
    ///
    /// # Example
    ///
    /// ```
    /// use rigtest::Preflight;
    /// let _ = Preflight::new().custom("mx_record", || async {
    ///     // … any async check returning Result<(), BoxError> …
    ///     Ok::<_, Box<dyn std::error::Error + Send + Sync>>(())
    /// });
    /// ```
    pub fn custom<F, Fut>(mut self, name: impl Into<Cow<'static, str>>, factory: F) -> Self
    where
    ...
    /// use rigtest::Preflight;
    /// let _ = Preflight::new()
    ///     .ssh("bastion", "deploy@bastion.example.com")
...
mod tests {
    ...
    fn tcp_probe_records_default_timeout() {
        ...
        assert_eq!(probes[0].timeout, Some(DEFAULT_PROBE_TIMEOUT));
    }
    ...
    fn timeout_overrides_last_probe() {
        ...
        assert_eq!(p.probes()[0].timeout, Some(Duration::from_millis(250)));
    }
    ...
    fn equals_pins_expected_value() {
        ...
        match &p.probes()[0].kind {
            ProbeKind::Env { expected, .. } => {
                assert_eq!(expected.as_deref(), Some("prod"));
            }
            _ => panic!("expected env probe"),
        }
    }
    ...
    fn timeout_after_chain_only_affects_just_added_probe() {
        let p = Preflight::new()
            ...
            .tcp("b", "1.2.3.4:2");
        assert_eq!(p.probes()[0].timeout, Some(Duration::from_millis(100)));
    ...
    fn dns_probe_records_default_timeout() {
        ...
        assert!(matches!(
            &probes[0].kind,
            ProbeKind::Dns { host } if host == "example.com"
        ));
        assert_eq!(probes[0].timeout, Some(DEFAULT_PROBE_TIMEOUT));
    }
    ...
    fn each_primitive_carries_its_documented_default_timeout() {
        ...
        #[cfg(feature = "http-client")]
        {
            ...
            assert_eq!(p.probes()[4].timeout, Some(DEFAULT_PROBE_TIMEOUT));

            #[cfg(all(feature = "ssh-client", unix))]
            {
                let p = p.ssh("ssh", "deploy@bastion");
                assert_eq!(p.probes()[5].timeout, Some(DEFAULT_PROBE_TIMEOUT));
            }
        }
    }

    #[test]
    fn custom_probe_defaults_to_no_timeout() {
        // `custom` is the only primitive without a framework-imposed
        // default — the framework cannot guess what work the closure
        // does, so the closure owns its own discipline.
        let p = Preflight::new().custom("noop", || async { Ok(()) });
        let probes = p.probes();
        assert_eq!(probes.len(), 1);
        assert_eq!(probes[0].name, "noop");
        assert!(matches!(probes[0].kind, ProbeKind::Custom { .. }));
        assert_eq!(probes[0].timeout, None);
    }
    ...
    #[test]
    fn builder_accepts_owned_strings_for_every_primitive() {
        // Regression guard: the builder must accept owned strings
        // (`String`, `format!(...)`) — not just `&'static str` literals
        // — so users can construct probe targets dynamically.
        let port = 8080;
        let p = Preflight::new()
            .tcp(format!("api_{port}"), format!("127.0.0.1:{port}"))
            .env(String::from("env_probe"), String::from("HOME"))
            .equals(String::from("/root"))
            .dns(format!("dns_{port}"), String::from("example.com"))
            .custom(format!("custom_{port}"), || async { Ok(()) });
        let probes = p.probes();
        assert_eq!(probes.len(), 4);
        assert_eq!(probes[0].name, "api_8080");
        assert!(matches!(
            &probes[0].kind,
            ProbeKind::Tcp { target } if target == "127.0.0.1:8080"
        ));
        match &probes[1].kind {
            ProbeKind::Env { var, expected } => {
                assert_eq!(var, "HOME");
                assert_eq!(expected.as_deref(), Some("/root"));
            }
            _ => panic!("expected env probe"),
        }
        assert!(matches!(
        ...
        {
            let p =
                Preflight::new().http(format!("http_{port}"), format!("http://127.0.0.1:{port}/"));
            match &p.probes()[0].kind {
                ProbeKind::Http { url, .. } => assert_eq!(url, "http://127.0.0.1:8080/"),
                _ => panic!("expected http probe"),
            }
        }
        ...
        {
            let p = Preflight::new()
                .ssh(format!("ssh_{port}"), format!("user@{}", "example.com"))
                .command(format!("test -d {}", "/opt/app"));
            match &p.probes()[0].kind {
                ProbeKind::Ssh { dest, command } => {
                    assert_eq!(dest, "user@example.com");
                    assert_eq!(command, "test -d /opt/app");
                }
                _ => panic!("expected ssh probe"),
            }
        }
